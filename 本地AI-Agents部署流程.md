...
| Phase 3：会话/消息产品化 | ✅ 已完成 | 已实现：消息复制、单条删除、重新生成、用户消息编辑重发、跨会话搜索（标题 + 消息内容）、Markdown / JSON / 纯文本导出（含可读工具结果段落）、回复多版本切换、SQLite migration 框架。 |
...
| Phase 6：工具/Agent Web 适配 | ✅ 已实现 | 已支持工具状态/参数展示、结构化工具结果持久化与工具时间线聚合；时间线支持 Step 编号、重试标记、失败节点和版本联动；刷新会话后仍能恢复历史搜索来源卡片；当前以单次问答为聚合边界，已满足现阶段需求。 |
| UI Phase 1：高级默认视觉 | ✅ 已实现 | Web 默认样式已升级为更中性、简洁的深色产品界面；主对话区、历史会话侧栏和右侧工作区完成第一轮视觉打磨，代码块、Markdown 和工具时间线统一为更克制的执行/阅读层次。 |
| UI Phase 1.x：动效与侧栏布局修补 | ✅ 已实现 | 已修复长会话标题与模型/日期重叠；新增全局 motion token、reduced-motion 护栏，并为消息进入、工具时间线、折叠块、版本切换、工作区目录/预览切换接入低调动效。 |
| UI Phase 2：外观设置 MVP | ✅ 已实现 | 右侧面板新增“外观”设置；支持正文字体、代码字体、自定义字体栈、正文/代码字号、阅读宽度、上下行间距和界面密度，并持久化到浏览器 localStorage。 |
| UI Phase 3/4：浅色线框空间风格 | ✅ 已实现 | 默认界面参考 Claude 与 AI Chatbox 调整为浅色纸面、细线分区、轻量控件和低阴影空间层次；侧栏、主对话、工作区、代码块、工具时间线和输入框已同步适配。 |
| 启动/关闭体验补丁 | ✅ 已实现 | quick-start 支持 BFF 日志、只停 Python BFF、keep-open 后输入 `q` 退出；Web 状态区支持关闭 Python 后端和重新连接。 |

> 2026-04-26 UI 样式补充：当前 Web UI 已完成 Phase 1 默认视觉升级，重点减少蓝紫渐变和重毛玻璃感，增强主对话区文档阅读体验、会话列表任务卡片质感和工作区文件浏览器质感。字体、代码字体、字号、阅读宽度和密度等可编辑外观设置仍作为后续 UI Phase 2 继续实现。

> 2026-04-26 UI 动效补充：当前 Web UI 已完成 Phase 1.x 动效接入，采用安静、低频的产品化动效；系统开启 reduced motion 时会关闭非必要动画。工作区目录进入/返回和文件预览切换通过 React class/key 触发，不引入额外动画库。

> 2026-04-26 UI 外观设置补充：当前 Web UI 已完成 Phase 2 外观设置 MVP，入口位于右侧面板“外观”；设置会即时更新 CSS variables，并保存到浏览器 localStorage。浏览器无法可靠读取系统完整字体列表，因此当前采用常用预设 + 自定义字体栈输入的方式支持本机已安装字体。阅读宽度、代码字体和上下行间距已接入实际对话渲染层，适合在三栏布局下直接调整阅读舒适度。

> 2026-04-26 UI 浅色线框风格补充：当前 Web UI 已完成 Phase 3/4 视觉重置，默认从深色渐变背景切换为浅色纸面界面；布局参考 Claude 的阅读留白和 AI Chatbox 的紧凑侧栏，控件以细线框、轻填充、低阴影为主，减少大面积高饱和色和重装饰。

> 2026-04-26 启动/关闭体验补充：`quick-start` 现在会把 Python BFF 日志写入 `data/logs/bff.log` / `data/logs/bff.err.log`，健康检查失败时输出日志尾部。Windows 可用 `scripts/quick-start.ps1 -StopBackend` 或 `scripts/stop-backend.ps1` 只关闭 Python BFF；Git Bash / WSL 可用 `scripts/quick-start.sh --stop-backend` 或 `scripts/stop-backend.sh`。需要启动窗口保持时可用 `-KeepOpen` / `--keep-open`，输入 `q` 后关闭 Python BFF/Web dev server，但不关闭 Docker 与 Ollama。Web 右侧“状态”区也提供“关闭 Python 后端”和“重新连接”。
...
> 也可使用一键启动脚本 `scripts/quick-start.ps1`，会自动检查 Python、Node.js、Docker、Ollama，启动 skill 服务、Python BFF，并启动 Web UI。
...
```powershell
scripts/quick-start.ps1 -KeepOpen
```

打开 Web UI 后，在该窗口输入 `q` 可关闭本次 Python BFF/Web dev server；Docker skill services 和 Ollama 会继续运行。
...
- `scripts/quick-start.ps1 -SkipFrontend` 可只启动 skill 服务和 BFF，便于单独调试 Web 前端；`-SkipCLI` 作为兼容别名仍可使用

### 6. 停止服务

只关闭 Python BFF（不关闭 Docker 和 Ollama）：

```powershell
scripts/quick-start.ps1 -StopBackend
# 或
...
```bash
scripts/quick-start.sh --stop-backend
# 或
scripts/stop-backend.sh
```

关闭 Docker skill services：
