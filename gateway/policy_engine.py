...
class PolicyEngine:
    ...
    def check(self, tool: str, params: dict[str, Any]) -> None:
        ...
        # write-only prefix check: block delete and rename-away from protected dirs
        if tool in {"file_delete", "file_rename"}:
            # file_delete uses "path"; file_rename uses "src" as the source
    ...
    def _check_path(self, path: str) -> None:
        ...
        # Normalise to an absolute POSIX path, resolving .. and . without OS calls.
        # Prepend "/" so relative paths are anchored before normalisation.
